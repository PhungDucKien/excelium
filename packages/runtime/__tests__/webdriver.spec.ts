...
import { createHeadlessChrome, createHeadlessFirefox, createStaticSite } from '@excelium/testkit';
  ...
  describe.each([
    ...
    describe('add selection', () => {
      ...
      it('should add to a multi select element', async () => {
        ...
        expect(((await driver.executeScript('return arguments[0].selectedOptions', element)) as any[]).length).toBe(0);
        await executor.doAddSelection('id=mult', 'label=Volvo');
        const selections = (await driver.executeScript('return arguments[0].selectedOptions', element)) as any[];
        ...
        expect(((await driver.executeScript('return arguments[0].selectedOptions', element)) as any[]).length).toBe(1);
        await executor.doAddSelection('id=mult', 'value=audi');
        expect(((await driver.executeScript('return arguments[0].selectedOptions', element)) as any[]).length).toBe(2);
      });
    });
