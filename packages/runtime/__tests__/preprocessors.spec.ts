...
import { composePreprocessors, interpolateScript, interpolateString, preprocessArray } from '../src/preprocessors';
...
describe('preprocessor composition', () => {
  ...
  it('should ignore preprocess if null is passed', () => {
    variables.set('a', 'a');
    const fn = jest.fn();
    FakeExecutor.prototype.doFake = composePreprocessors(interpolateString, null, fn);
    const exec = new FakeExecutor();
    exec.init({ variables });
    exec.doFake('${a}', '${a}');
    expect(fn).toHaveBeenCalledWith('a', '${a}');
