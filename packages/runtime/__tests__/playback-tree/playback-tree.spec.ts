...
import { createCommandNodesFromCommandStack, createPlaybackTree } from '../../src/playback-tree';
...
describe('Control Flow', () => {
  describe('Process', () => {
    describe('Linked List Validation', () => {
      test('nodes contain command references and levels', () => {
        const input = [createCommand('command1'), createCommand('command2')];
        const stack = createCommandNodesFromCommandStack(input);
      ...
      test('while-end', () => {
        const input = [createCommand(ControlFlowCommandNames.while), createCommand(ControlFlowCommandNames.end)];
        const stack = createCommandNodesFromCommandStack(input);
        expect(stack[0].next).toBeUndefined();
        expect(stack[0].right).toEqual(stack[1]);
      ...
      test('while-command-end', () => {
        const input = [createCommand(ControlFlowCommandNames.while), createCommand('command'), createCommand(ControlFlowCommandNames.end)];
        const stack = createCommandNodesFromCommandStack(input);
        expect(stack[0].next).toBeUndefined();
        expect(stack[0].right).toEqual(stack[1]);
        expect(stack[0].left).toEqual(stack[2]);
      ...
      test('times-command-end', () => {
        const input = [createCommand(ControlFlowCommandNames.times), createCommand('command'), createCommand(ControlFlowCommandNames.end)];
        const stack = createCommandNodesFromCommandStack(input);
        expect(stack[0].next).toBeUndefined();
        expect(stack[0].right).toEqual(stack[1]);
        expect(stack[0].left).toEqual(stack[2]);
      ...
      test('forEach-command-end', () => {
        const input = [createCommand(ControlFlowCommandNames.forEach), createCommand('command'), createCommand(ControlFlowCommandNames.end)];
        const stack = createCommandNodesFromCommandStack(input);
        expect(stack[0].next).toBeUndefined();
        expect(stack[0].right).toEqual(stack[1]);
        expect(stack[0].left).toEqual(stack[2]);
  ...
  describe('Processed', () => {
    it('knows if there are control flow commands within the command stack', () => {
      const input = [createCommand(ControlFlowCommandNames.do), createCommand('command'), createCommand(ControlFlowCommandNames.repeatIf)];
