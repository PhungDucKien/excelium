...
export async function resolveDriverUrl({
  ...
  switch (browser) {
    ...
    case 'chrome': {
      return `https://chromedriver.storage.googleapis.com/${await getChromedriverVersion(version)}/chromedriver_${getChromedriverPlatformName(
        platform
      )}.zip`;
    }
    case 'firefox': {
      return `https://github.com/mozilla/geckodriver/releases/download/v${getGeckodriverVersion(version)}/geckodriver-v${getGeckodriverVersion(
        version
      )}-${getGeckodriverPlatformName(platform, arch)}.${platform === 'win32' ? 'zip' : 'tar.gz'}`;
    }
  }
}

export function resolveDriverName({ browser, platform, version }: { browser: Browser; platform: Platform; version: string }) {
...
async function getChromedriverVersion(version: string) {
  ...
  const res = await fetch(`https://chromedriver.storage.googleapis.com/LATEST_RELEASE_${major}`);
